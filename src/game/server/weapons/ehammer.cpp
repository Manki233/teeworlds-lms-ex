...
CEHammer::CEHammer(CCharacter *pOwnerChar) :
	CWeapon(pOwnerChar)
{
	...
	m_FireDelay = g_pData->m_Weapons.m_aId[WEAPON_HAMMER].m_Firedelay * 3;
}

void CEHammer::Fire(vec2 Direction)
{
	...
	GameWorld()->CreateExplosion(HammerHitPos, ClientID, WEAPON_HAMMER, GetWeaponID(), 3, false);
	...
	if(Hits)
	{
		...
		if(!TuneZone)
			FireDelay = GameServer()->Tuning()->m_HammerHitFireDelay * 3;
		else
			FireDelay = GameServer()->TuningList()[TuneZone].m_HammerHitFireDelay * 3;
